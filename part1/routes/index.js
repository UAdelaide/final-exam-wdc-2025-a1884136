  ...
  try {
    const [rows] = await pool.query(
      'SELECT u.username, COUNT(wr.request_id) AS completed_walks
       FROM Users u JOIN WalkApplications wa ON u.user_id = wa.walker_id JOIN WalkRequests wr ON wa.request_id = wr.request_id WHERE u.role = 'walker' AND wa.status = 'accepted' AND wr.status = 'completed' GROUP BY u.username');
  }
});
